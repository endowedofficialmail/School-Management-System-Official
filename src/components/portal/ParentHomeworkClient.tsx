...
export default function ParentHomeworkClient({
  homework,
}: {
  homework: Homework[]
}) {
          ...
          return (
            <Card key={hw.id} className={hw.isDone ? 'bg-emerald-50' : ''}>
              <CardContent className="p-4 flex items-start justify-between gap-3">
                <div>
                  <p className={`font-semibold ${hw.isDone ? 'line-through' : ''}`}>{hw.title}</p>
                ...
                <Button
                  ...
                  variant="outline"
                  disabled
                  title="Only your child can mark homework as done"
                >
                  {hw.isDone ? <><Check className="h-3 w-3 mr-1" /> Done</> : 'Not Done'}
                </Button>
              </CardContent>
            </Card>
          )
