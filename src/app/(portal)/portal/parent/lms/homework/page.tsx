...
import ParentHomeworkClient from '@/components/portal/ParentHomeworkClient'
...
export default async function ParentHomeworkPage({
  ...
  return (
    <div className="space-y-4">
      {data.students.length > 1 && (
        <div className="flex gap-2 flex-wrap">
          {data.students.map((s) => (
            <Link
              key={s.student.id}
              href={`/portal/parent/lms/homework?studentId=${s.student.id}`}
      ...
      <ParentHomeworkClient homework={homework} />
    </div>
  )
}
