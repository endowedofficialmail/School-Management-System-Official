...
export default function PrintClassRollSlipsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const filtered = list.filter((s) => s.isValid && s.student.class.id === classId)
      const full = await Promise.all(filtered.map((s) => getRollSlipById(s.id)))
  ...
  if (slips.length === 0) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', fontFamily: 'Georgia, serif', color: 'red' }}>
        No valid roll slips found for this class.
      </div>
    )
  }
